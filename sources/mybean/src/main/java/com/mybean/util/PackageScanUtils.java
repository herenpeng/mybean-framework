...
public class PackageScanUtils {
    ...
    /**
     * 程序编译之后.class文件所在的文件路径：classes\，用于截取全限定类名
     */
    private final static String PREFIX = "classes" + SYSTEM_SEPARATOR;
    /**
     * .class文件的后缀名
     */
     ...
     */
    private static void getClassNameByFile(File file) {
        if (!file.exists()) {
            return;
        }
        if (file.isFile()) {
            ...
            if (path.endsWith(SUFFIX)) {
                String className = path.substring(path.lastIndexOf(PREFIX) + PREFIX.length(), path.lastIndexOf(SUFFIX));
